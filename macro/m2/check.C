...
void func1( int month, int runnum){
  ...
  gROOT->Reset();
  TFile *f = new TFile(Form("%s/analyzer_%s/rootfile/run%05d_DstKuramaHodoscope_trigf19.root", anadir.Data(),Month[month], runnum),"READ");
  TTree *khodo;
  ...
  //
  // Hist Declaretion -------------------------------------------------------------
  TH1D *masssquare = new TH1D(Form("m2_run%05d",runnum),"m2",200,-0.4,1.4);
...
void check(){
  ...
  TString pdf = "/home/had/suharu/work/e40/ana/pdf/m2/check_trigf19.pdf";
