...
//Reset ROOT and connect tree file
   ...
   TString pdf = Form("%s/pdf/trigger/SAC_Efficiency_BH2TOF.pdf", anadir.Data());
//   TFile *f = new TFile(Form("%s/analyzer_%s/rootfile/trigf19_tofht.root", anadir.Data(),Month[month]),"READ");
   TFile *f = new TFile(Form("%s/analyzer_%s/rootfile/run05078_DstKuramaHodoscope.root", anadir.Data(),Month[month]),"READ");
   TTree *khodo;
