...
void Mtx_Banch3(int month, int runnum, int filesel=1){
...
//  TString filein1=Form("%s/analyzer_%s/param/MATRIXSFT/SFT_table.txt.2018Jun.3_1",anadir.Data(),Month[month] ); 
  TString filein1=Form("%s/analyzer_%s/param/MATRIXSFT/SFT_Newtable.txt.2019Jan.1_2",anadir.Data(),Month[month] ); 
  ...
  //  TString anadir=Form("%s/work/e40/ana",std::getenv("HOME")); 
  TString pdf = Form("%s/pdf/trigger/Mtx_Banch3_run%05d_%d.pdf", anadir.Data(),runnum,filesel);
...
//    TFile *f = new TFile(Form("%s/analyzer_%s/rootfile/run%05d_Matrix.root", anadir.Data(),Month[month], runnum),"READ");
  ...
  for (Long64_t s=0; s<nentries;s++){
    ...
    for(int i=0; i<Hist1Max; i++){
      Hist1[i]->Draw();
      c1->Print(pdf);
      c1->Print(Form("%s/Mtx_Banch3_run%05d_Hist1_%03d_%d.pdf",pdfDhire.Data(),runnum,i, filesel));
    }
    ...
    c1->Print(pdf);
    c1->Print(Form("%s/Mtx_Banch3_run%05d_Graph_%d.pdf",pdfDhire.Data(),runnum, filesel));
    ...
    c1->Print(pdf);
    c1->Print(Form("%s/Mtx_Banch3_run%05d_Graph_Sigma_%d.pdf",pdfDhire.Data(),runnum, filesel));
