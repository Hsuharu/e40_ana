...
void Mtx_Pat_Compare(int month,int runnum, int bin, int file, int matrixnum1 =2, int matrixnum2 =1){
...
//Reset ROOT and connect tree file
   ...
   TString pdf = Form("%s/pdf/trigger/Mtx_Pat_Compare%d_bin%d_%d_run%05d.pdf", anadir.Data(),file, bin, matrixnum,runnum);
   TString pdf1 = Form("%s/pdf/trigger/Mtx_Compare%d_bin%d_%d_run%05d.pdf", anadir.Data(),file, bin, matrixnum,runnum);
...
//   Hist1Max = 5151;
//   Hist2Max = 1214;
   Hist1Max = 34 + (int)Mtx_prm.size()*17;
   Hist2Max = 8 + (int)Mtx_prm.size()*4 + 2;
   chisqr = 50;
   TH1D *Hist1[Hist1Max];
   TH2D *Hist2[Hist2Max];
    ...
    Hist2[8+Mtx_prm.size()*4]= new TH2D("tofsegKurama[0] % vpseg[1] Sigma w/Matrix","tofsegKurama[0] % vpseg[1] Sigma w/Matrix" ,NumOfSegSCH,0,NumOfSegSCH,NumOfSegTOF,0,NumOfSegTOF);
    Hist2[8+Mtx_prm.size()*4+1]= new TH2D("tofsegKurama[0] % vpseg[1] Sigma","tofsegKurama[0] % vpseg[1] Sigma" ,NumOfSegSCH,0,NumOfSegSCH,NumOfSegTOF,0,NumOfSegTOF);
