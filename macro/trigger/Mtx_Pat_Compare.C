...
void Mtx_Pat_Compare(int month,int runnum, int bin=256, int file=2, int matrixnum1 =2, int matrixnum2 =1){
  ...
  TString pdf = Form("%s/pdf/trigger/Mtx_Pat_Compare%d_bin%d_matrix%d_%d_run%05d.pdf", anadir.Data(),file, bin, matrixnum1, matrixnum2,runnum);
  TString pdf1 = Form("%s/pdf/trigger/Mtx_Compare%d_bin%d_matrix%d_%d_run%05d.pdf", anadir.Data(),file, bin, matrixnum1, matrixnum2,runnum);
  ...
  // Param Vector Dif ----------------------------------------------------------------------
  std::vector<std::vector<int>> Mtx_prm1; //size() = 301
  std::vector<std::vector<int>> Mtx_prm2; //size() = 301
  std::string line1;
  std::string line2;
  int preSCH=0;
  std::vector<std::vector<int>> sch_tof; 
  std::vector<int> SCH_Seg1; 
  std::vector<int> TOF_Min1; 
  std::vector<int> TOF_Max1; 
  ...
  for(int i=0; i<Mtx_prm1.size(); i++){
    ...
    if(i==0){
      SCH_Seg1.push_back( Mtx_prm1.at(i).at(0) );
      TOF_Min1.push_back( Mtx_prm1.at(i).at(1) );
    }else{
      if(i==Mtx_prm1.size()-1){
        TOF_Max1.push_back( Mtx_prm1.at(i).at(1) );
      }else if(Mtx_prm1.at(i).at(0)!=Mtx_prm1.at(i-1).at(0)){
        SCH_Seg1.push_back( Mtx_prm1.at(i).at(0) );
        TOF_Max1.push_back( Mtx_prm1.at(i-1).at(1) );
        TOF_Min1.push_back( Mtx_prm1.at(i).at(1) );
      }
    }
  }
  ...
  if( !eq3(SCH_Seg1.size(),TOF_Min1.size(),TOF_Max1.size()) ){
  ...
  for(int i=0; i<Mtx_prm2.size(); i++){
    //        std::cout << "SCH=" << Mtx_prm.at(i).at(0)  << "\t" << "TOF="  <<  Mtx_prm.at(i).at(1)  << "\t"  << "SFT_Min=" << Mtx_prm.at(i).at(2)  << "\t"  << "SFT_Max=" << Mtx_prm.at(i).at(3)  << std::endl;
    if(i==0){
      SCH_Seg2.push_back( Mtx_prm2.at(i).at(0) );
      TOF_Min2.push_back( Mtx_prm2.at(i).at(1) );
    }else{
      if(i==Mtx_prm2.size()-1){
        TOF_Max2.push_back( Mtx_prm2.at(i).at(1) );
      }else if(Mtx_prm2.at(i).at(0)!=Mtx_prm2.at(i-1).at(0)){
        SCH_Seg2.push_back( Mtx_prm2.at(i).at(0) );
        TOF_Max2.push_back( Mtx_prm2.at(i-1).at(1) );
        TOF_Min2.push_back( Mtx_prm2.at(i).at(1) );
      }
    }
  }
  ...
  //   Hist1Max = 5151;
  //   Hist2Max = 1214;
  Hist1Max = 34 + (int)Mtx_prm2.size()*17;
  Hist2Max = 8 + (int)Mtx_prm2.size()*4 + 2;
  chisqr = 50;
  TH1D *Hist1[Hist1Max];
  TH2D *Hist2[Hist2Max];
  ...
  for(int l=0; l < Mtx_prm2.size(); l++){
    Hist1[34+Mtx_prm2.size()*0 +l]= new TH1D(Form("sftxseg Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),Form("sftx Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),bin,0,NumOfSegSFT_X);
    Hist1[34+Mtx_prm2.size()*1 +l]= new TH1D(Form("sftxseg Cut2 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),Form("sftx Cut2 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),bin,0,NumOfSegSFT_X);
    Hist1[34+Mtx_prm2.size()*2 +l]= new TH1D(Form("sftxseg Cut3 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),Form("sftx Cut3 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),bin,0,NumOfSegSFT_X);
    Hist1[34+Mtx_prm2.size()*3 +l]= new TH1D(Form("sftxseg Cut4 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),Form("sftx Cut4 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),bin,0,NumOfSegSFT_X);
    Hist1[34+Mtx_prm2.size()*4 +l]= new TH1D(Form("sftxseg Cut5 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),Form("sftx Cut5 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),bin,0,NumOfSegSFT_X);
    Hist1[34+Mtx_prm2.size()*1 +l]->SetLineColor(8); 
    ...
    Hist1[34+Mtx_prm2.size()*8 +l]= new TH1D(Form("m2 Cut4 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),Form("m2 Cut4 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),100,-0.4,1.4);
    Hist1[34+Mtx_prm2.size()*9 +l]= new TH1D(Form("m2 Cut5 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),Form("m2 Cut5 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),100,-0.4,1.4);
    Hist1[34+Mtx_prm2.size()*10+l]= new TH1D(Form("pKurama Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),Form("pKurama Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),100,0,2);
    Hist1[34+Mtx_prm2.size()*11+l]= new TH1D(Form("pKurama Cut2 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),Form("pKurama Cut2 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),100,0,2);
    ...
    Hist1[34+Mtx_prm2.size()*15+l]= new TH1D(Form("MissMass Sigma Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),Form("MissMass Sigma Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),100,0.8,1.4);
    Hist1[34+Mtx_prm2.size()*16+l]= new TH1D(Form("sftxseg all Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),Form("sftx all Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1),bin,0,NumOfSegSFT_X);
  }                         
  ...
  for(int l=0; l < Mtx_prm2.size(); l++){
    Hist2[8+Mtx_prm2.size()*0+l]= new TH2D(Form("pKurama %% m2 Cut2 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1), Form("pKurama %% m2 Cut2 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1)   ,100,-0.4,1.4,100,0,2);
    Hist2[8+Mtx_prm2.size()*1+l]= new TH2D(Form("pKurama %% m2 Cut3 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1), Form("pKurama %% m2 Cut3 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1)   ,100,-0.4,1.4,100,0,2);
    Hist2[8+Mtx_prm2.size()*2+l]= new TH2D(Form("pKurama %% m2 Cut4 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1), Form("pKurama %% m2 Cut4 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1)   ,100,-0.4,1.4,100,0,2);
    Hist2[8+Mtx_prm2.size()*3+l]= new TH2D(Form("pKurama %% m2 Cut5 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1), Form("pKurama %% m2 Cut5 Sch[%d]-Tof[%d]",Mtx_prm2.at(l).at(0)+1,Mtx_prm2.at(l).at(1)+1)   ,100,-0.4,1.4,100,0,2);
  }                                         

  Hist2[8+Mtx_prm2.size()*4]= new TH2D("tofsegKurama[0] % vpseg[1] Sigma w/Matrix","tofsegKurama[0] % vpseg[1] Sigma w/Matrix" ,NumOfSegSCH,0,NumOfSegSCH,NumOfSegTOF,0,NumOfSegTOF);
  Hist2[8+Mtx_prm2.size()*4+1]= new TH2D("tofsegKurama[0] % vpseg[1] Sigma","tofsegKurama[0] % vpseg[1] Sigma" ,NumOfSegSCH,0,NumOfSegSCH,NumOfSegTOF,0,NumOfSegTOF);
  ...
  for (Long64_t s=0; s<nentries;s++) {
    ...
    for(int l=0; l < Mtx_prm2.size(); l++){
      double m = 0;
      double n = 0;
      m = (double)Mtx_prm2.at(l).at(1);
      n = (double)Mtx_prm2.at(l).at(0);
      if( vpseg[1]==n && tofsegKurama[0]-1.==m ){
        for(int j=0; j<sftx_unhits; j++){
          Hist1[34+Mtx_prm2.size()*16+l]->Fill(sftx_uhitpat[j]);
        }
      }
    }
    ...
    if(m2[0]>0.15 && m2[0]<0.35 && qKurama[0]>0){
      if( vtx[0]>-20 && vtx[0]<20 && vty[0]>-20 && vty[0]<20 && vtz[0]>-200 && vtz[0]<200 ){
        Hist2[8+Mtx_prm2.size()*4+1]->Fill(vpseg[1],tofsegKurama[0]-1.);
      }//Cut5
    }//Cut4
    ...
    for(int l=0; l < Mtx_prm2.size(); l++){
      ...
      m = (double)Mtx_prm2.at(l).at(1);
      n = (double)Mtx_prm2.at(l).at(0);
      min = (double)Mtx_prm2.at(l).at(2);
      max = (double)Mtx_prm2.at(l).at(3) + 1;
      if( vpseg[1]==n && tofsegKurama[0]-1.==m ){
        Hist1[34+Mtx_prm2.size()*0 +l]->Fill(sftxsegKurama);
        Hist1[34+Mtx_prm2.size()*5 +l]->Fill(m2[0]); 
        Hist1[34+Mtx_prm2.size()*10 +l]->Fill(pKurama[0]); 
        if(m2[0]>-0.15 && m2[0]<0.15){
          Hist1[26]->Fill(pKurama[0]);
          Hist1[27]->Fill(m2[0]);
          Hist1[34+Mtx_prm2.size()*1 +l]->Fill(sftxsegKurama); 
          Hist1[34+Mtx_prm2.size()*6 +l]->Fill(m2[0]); 
          Hist1[34+Mtx_prm2.size()*11 +l]->Fill(pKurama[0]); 
          Hist2[4]->Fill(m2[0],pKurama[0]);
          Hist2[8+Mtx_prm2.size()*0+l]->Fill(m2[0],pKurama[0]);
        }//Cut2
        if(m2[0]>0.6 && m2[0]<1.0 && qKurama[0]>0){
          Hist1[28]->Fill(pKurama[0]);
          Hist1[29]->Fill(m2[0]);
          Hist1[34+Mtx_prm2.size()*2 +l]->Fill(sftxsegKurama); 
          Hist1[34+Mtx_prm2.size()*7 +l]->Fill(m2[0]); 
          Hist1[34+Mtx_prm2.size()*12 +l]->Fill(pKurama[0]); 
          Hist2[5]->Fill(m2[0],pKurama[0]);
          Hist2[8+Mtx_prm2.size()*1+l]->Fill(m2[0],pKurama[0]);
        }//Cut3
        if(m2[0]>0.15 && m2[0]<0.35 && qKurama[0]>0){
          Hist1[30]->Fill(pKurama[0]);
          Hist1[31]->Fill(m2[0]);
          Hist1[34+Mtx_prm2.size()*3 +l]->Fill(sftxsegKurama); 
          Hist1[34+Mtx_prm2.size()*8 +l]->Fill(m2[0]); 
          Hist1[34+Mtx_prm2.size()*13 +l]->Fill(pKurama[0]); 
          Hist2[6]->Fill(m2[0],pKurama[0]);
          Hist2[8+Mtx_prm2.size()*2+l]->Fill(m2[0],pKurama[0]);
          if( vtx[0]>-20 && vtx[0]<20 && vty[0]>-20 && vty[0]<20 && vtz[0]>-200 && vtz[0]<200 ){
            if(sftxsegKurama>min&&sftxsegKurama<max){
              Hist1[34+Mtx_prm2.size()*15 +l]->Fill(MissMass[0]); 

            }
            if( MissMass[0]>1.15 && MissMass[0]<1.25 ){
              Hist1[32]->Fill(pKurama[0]);
              Hist1[33]->Fill(m2[0]);
              Hist1[34+Mtx_prm2.size()*4 +l]->Fill(sftxsegKurama); 
              Hist1[34+Mtx_prm2.size()*9 +l]->Fill(m2[0]); 
              Hist1[34+Mtx_prm2.size()*14 +l]->Fill(pKurama[0]); 
              Hist2[7]->Fill(m2[0],pKurama[0]);
              Hist2[8+Mtx_prm2.size()*3+l]->Fill(m2[0],pKurama[0]);
              Hist2[8+Mtx_prm2.size()*4]->Fill(vpseg[1],tofsegKurama[0]-1.);
            }       
          }//Cut5
        }//Cut4
      }
  ...
  //         double TOF_Max_y2;
  //
  //         x1 =  (double)SCH_Seg2.at(i);
  //         x2 =  (double)(SCH_Seg2.at(i)+1);
  //
  //         TOF_Min_y1 =(double)TOF_Min2.at(i);
  //         TOF_Min_y2 =(double)TOF_Min2.at(i);
  //
  ...
  //       }
  //       c1->Print(pdf);
  //       c1->Print(Form("%s/Mtx_Pat_Compar_bin%d_run%05d_Hist2_box_%04d.pdf",pdfDhire.Data(),bin,runnum,j));
  //     }
  //   }
...
//            double TOF_Max_y2;
            ...
            TOF_Min_y1 =(double)TOF_Min1.at(i);
            TOF_Min_y2 =(double)TOF_Min1.at(i);

            TOF_Max_y1 =  (double)TOF_Max1.at(i)+1;
            TOF_Max_y2 =  (double)TOF_Max1.at(i)+1;
            ...
            }else if(i==SCH_Seg1.size()-1){
              TLine *MtxGateMin_Y1 = new TLine(x2,TOF_Min_y1,x2,TOF_Max_y1);
              MtxGateMin_Y1->SetLineColor(1);
              MtxGateMin_Y1->SetLineWidth(1);
              MtxGateMin_Y1->Draw("same");
            }else{
              if(TOF_Min1.at(i)-TOF_Min1.at(i-1)!=0){
                TLine *MtxGate_Y1_1 = new TLine(x1,(double)TOF_Min1.at(i-1),x1,TOF_Min_y1);
                MtxGate_Y1_1->SetLineColor(1);
                MtxGate_Y1_1->SetLineWidth(1);
                MtxGate_Y1_1->Draw("same");
              }
              if(TOF_Max1.at(i)-TOF_Max1.at(i-1)!=0){
                TLine *MtxGate_Y2_1 = new TLine(x1,(double)TOF_Max1.at(i-1)+1,x1,TOF_Max_y1);
  ...
  for(int l=0; l < Mtx_prm2.size(); l++){
    ...
    Hist1[34+Mtx_prm2.size()*2 +l]->Draw("same");
    Hist1[34+Mtx_prm2.size()*3 +l]->Draw("same");
    ...
    TLine *MtxSftXGateMin1 = new TLine(min1,y1,min1,y2);
    TLine *MtxSftXGateMax1 = new TLine(max1,y1,max1,y2);
    MtxSftXGateMin->SetLineColor(2);
    MtxSftXGateMax->SetLineColor(2);
    MtxSftXGateMin->SetLineWidth(1);
    MtxSftXGateMax->SetLineWidth(1);
    MtxSftXGateMin1->SetLineColor(1);
    MtxSftXGateMax1->SetLineColor(1);
    MtxSftXGateMin1->SetLineWidth(1);
    MtxSftXGateMax1->SetLineWidth(1);
    ...
    c2->cd(3);
    Hist1[34+Mtx_prm2.size()*5 +l]->Draw();
