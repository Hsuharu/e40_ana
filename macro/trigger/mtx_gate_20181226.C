...
//
void mtx_gate_20181226(int month, int runnum){
//////////////////////////////////////////////////////////
...
//-Sft ----------------
  TH1D *SftX_U_Nhits = new TH1D("SftX U Nhits","Sft U Nhits",50,0,50);
  TH1D *SftX_D_Nhits = new TH1D("SftX D Nhits","Sft D Nhits",50,0,50);
  TH1D *SftXNhits = new TH1D("SftNhits","SftNhits",50,0,50);
  TH1D *SftX_U_Hitpat = new TH1D("Sft U Hitpat","Sft U Hitpat",256*2,0,256);
  TH1D *SftX_D_Hitpat = new TH1D("Sft D Hitpat","Sft D Hitpat",256*2,0,256);
  TH1D *SftXHitpat = new TH1D("SftHitpat","SftHitpat",256*2,0,256);
  TH1D *SftX_U_Tdc = new TH1D("Sft U Tdc","Sft U Tdc",1000,0,1000);
  TH1D *SftX_D_Tdc = new TH1D("Sft D Tdc","Sft D Tdc",1000,0,1000);
  TH1D *SftXTdc = new TH1D("SftTdc","SftTdc",1000,0,1000);
...
//           TofMtCut[i]->Fill(tofmt[i][j]);
     ...
     //SftX ----
     for(int i=0; i<94; i++){
       SftX_U_Hitpat->Fill(sftx_uhitpat[i]);
       SftXHitpat->Fill(sftx_uhitpat[i]);
     }
     for(int i=0; i<49; i++){
       SftX_D_Hitpat->Fill(sftx_dhitpat[i]+0.5);
       SftXHitpat->Fill(sftx_dhitpat[i]+0.5);
     }
     SftX_U_Nhits->Fill(sftx_unhits);
     SftX_D_Nhits->Fill(sftx_dnhits);
     SftXNhits->Fill(sftx_unhits);
     SftXNhits->Fill(sftx_dnhits);
     for(int i=0; i<NumOfSegSFT_X; i++){
       for(int j=0; j<16; j++){
         SftX_U_Tdc->Fill(sftx_utdc[i][j]);
         SftX_D_Tdc->Fill(sftx_dtdc[i][j]);
...
//Sft Draw ----
   ...
   SftX_U_NhitsCut->Draw();
   c1->Print(pdf);
   SftX_D_NhitsCut->Draw();
   c1->Print(pdf);
   SftXNhitsCut->Draw();
   c1->Print(pdf);
   SftX_U_HitpatCut->Draw();
   c1->Print(pdf);
   SftX_D_HitpatCut->Draw();
   c1->Print(pdf);
   SftXHitpatCut->Draw();
   c1->Print(pdf);
   SftX_U_TdcCut->Draw();
...
//Sft-Tof Draw ----
   SftXTof->Draw();
   c1->Print(pdf);
   MaxBinValue=SftXTof->GetXaxis()->GetBinCenter(SftXTof->GetMaximumBin());
   SftXTof->SetAxisRange(MaxBinValue-50,MaxBinValue+70,"X");
   SftXTof->Draw();
   c1->Print(pdf);
...
// Time Calib by KaonTime to 0----------------------------------------------------------------------
   ...
   double MaximumBinSch=0.;
   double SchTofKTime0=0.;
   MaximumBinSch=SchTofKCut->GetXaxis()->GetBinCenter(SchTofKCut->GetMaximumBin());
   SchTofKCut->Fit("FitFunc1","","",MaximumBinSch-5,MaximumBinSch+5);
   SchTofKCut->SetAxisRange(MaximumBinSch-50,MaximumBinSch+70,"X");
   SchTofKCut->Draw();
   SchTofKTime0=FitFunc1->GetParameter(1);
   c1->Print(pdf);

   SchTofPiCut->SetAxisRange(MaximumBinSch-50,MaximumBinSch+70,"X");
   SchTofPiCut->Draw();
   c1->Print(pdf);
   SchTofPCut->SetAxisRange(MaximumBinSch-50,MaximumBinSch+70,"X");
   SchTofPCut->Draw();
   c1->Print(pdf);

// Time Calib by KaonTime to 0----------------------------------------------------------------------
   double SftXTofKTime0=0.;
   double MaximumBinSftX=0.;
   MaximumBinSftX=SftXTofKCut->GetXaxis()->GetBinCenter(SftXTofKCut->GetMaximumBin());
   SftXTofKCut->Fit("FitFunc1","","",MaximumBinSftX-5,MaximumBinSftX+5);
   SftXTofKCut->SetAxisRange(MaximumBinSftX-50,MaximumBinSftX+70,"X");
   SftXTofKCut->Draw();
   SftXTofKTime0=FitFunc1->GetParameter(1);
   c1->Print(pdf);

   SftXTofPiCut->SetAxisRange(MaximumBinSftX-50,MaximumBinSftX+70,"X");
   SftXTofPiCut->Draw();
   c1->Print(pdf);
   SftXTofPCut->SetAxisRange(MaximumBinSftX-50,MaximumBinSftX+70,"X");
   SftXTofPCut->Draw();
   c1->Print(pdf);
...
// Sch-Tof KTime0 ------
   ...
   c1->Print(pdf);
   SchTofCut3_m2->SetAxisRange(MaximumBinSch-50,MaximumBinSch+70,"X");
   SchTofCut3_m2->Draw("colz");
   c1->Print(pdf);
   SchTofCut3_pKurama->SetAxisRange(MaximumBinSch-50,MaximumBinSch+70,"X");
   SchTofCut3_pKurama->Draw("colz");
   c1->Print(pdf);
   SchTofPiCut_KTime0_pKurama->Draw("colz");
   c1->Print(pdf);
   SchTofKCut_KTime0_pKurama->Draw("colz");
   c1->Print(pdf);
   SchTofPCut_KTime0_pKurama->Draw("colz");
   c1->Print(pdf);
...
// SftX-Tof KTime0 ------
   SftXTofPiCut_KTime0->Draw();
   c1->Print(pdf);
   SftXTofKCut_KTime0->Draw();
   c1->Print(pdf);
   SftXTofPCut_KTime0->Draw();
   c1->Print(pdf);
//   SftXTofCut3_m2->SetAxisRange(MaximumBinSftX-50,MaximumBinSftX+70,"X");
//   SftXTofCut3_m2->Draw("colz");
//   c1->Print(pdf);
//   SftXTofCut3_pKurama->SetAxisRange(MaximumBinSftX-50,MaximumBinSftX+70,"X");
//   SftXTofCut3_pKurama->Draw("colz");
//   c1->Print(pdf);
//   SftXTofPiCut_KTime0_pKurama->Draw("colz");
//   c1->Print(pdf);
//   SftXTofKCut_KTime0_pKurama->Draw("colz");
//   c1->Print(pdf);
//   SftXTofPCut_KTime0_pKurama->Draw("colz");
//   c1->Print(pdf);
//
//   gPad->SetLogy(1);
//   SftXTofCut3_KTime0->Draw();
//   c1->Print(pdf);
//   SftXTofCut3_KTime0_GOMI->Draw();
//   c1->Print(pdf);
//   gPad->SetLogy(0);

   SftXTofPCut_KTime0->SetStats(0);
   SftXTofPiCut_KTime0->SetStats(0);
   SftXTofKCut_KTime0->SetStats(0);
   SftXTofPCut_KTime0->SetLineColor(3);
   SftXTofPCut_KTime0->Draw();
   SftXTofKCut_KTime0->Draw("same");
   SftXTofPiCut_KTime0->SetLineColor(2);
   SftXTofPiCut_KTime0->Draw("same");

   Leg2->AddEntry(SftXTofPiCut_KTime0, "Pion"  ,"l");
   Leg2->AddEntry(SftXTofKCut_KTime0,  "Kaon"  ,"l");
   Leg2->AddEntry(SftXTofPCut_KTime0,  "Proton","l");
   Leg2->Draw();
   c1->Print(pdf);
   c1->Print(Form("%s/mtx_banch_20181226_run%05d_DistbyMass.pdf",pdfDhire.Data(),runnum));
