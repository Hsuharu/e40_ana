...
void Mtx_Pat_Make(int month,int runnum, int bin, int matrixnum =2){
...
//Reset ROOT and connect tree file
   ...
   TString pdf = Form("%s/pdf/trigger/Mtx_Pat_Make_bin%d_%d_run%05d.pdf", anadir.Data(), bin, matrixnum,runnum);
   TString pdf1 = Form("%s/pdf/trigger/Mtx_Check_bin%d_%d_run%05d.pdf", anadir.Data(), bin, matrixnum,runnum);
...
//     c1->Print(Form("%s/Mtx_Pat_Make_run%05d_Hist2_colz_%04d.pdf",pdfDhire.Data(),runnum,i));
   ...
   for(int j=0; j<Hist2Max; j++){
     if(j==1 || j==3 || j ==8+Mtx_prm.size()*4 || j ==8+Mtx_prm.size()*4+1){
       ...
       c1->Print(pdf);
       c1->Print(Form("%s/Mtx_Pat_Make_bin%d_run%05d_Hist2_box_%04d.pdf",pdfDhire.Data(),bin,runnum,j));
     }
   }
