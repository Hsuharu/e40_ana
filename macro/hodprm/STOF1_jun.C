...
void STOF1_jun( int month, int runnum){
////////////////////////////////////////////////////////////
...
//   }
   ...
   TH1D *TOFUT0[NumOfSegTOF]; 
   TH1D *TOFDT0[NumOfSegTOF]; 
   TH1D *TOFUTns[NumOfSegTOF]; 
   TH1D *TOFDTns[NumOfSegTOF]; 
   TH1D *TOFMT[NumOfSegTOF]; 
   TH1D *TOFUMIP[NumOfSegTOF]; 
   TH1D *TOFDMIP[NumOfSegTOF]; 
   TH1D *TOFUBG[NumOfSegTOF]; 
   TH1D *TOFDBG[NumOfSegTOF]; 
   TH1D *TOFUdE[NumOfSegTOF]; 
   TH1D *TOFDdE[NumOfSegTOF]; 
   TH2D *SHist[NumOfSegTOF]; 
      for (int i=0; i<NumOfSegTOF;i++) {
        TOFUT[i] = new TH1D(Form("TOF_%dUT",i+1),Form("TOF_%dUT",i+1),2000,200000,400000);
        TOFDT[i] = new TH1D(Form("TOF_%dDT",i+1),Form("TOF_%dDT",i+1),5000,200000,400000);
        TOFUT0[i] = new TH1D(Form("TOF_%dUT0",i+1),Form("TOF_%dUT0",i+1),2000,-100000,100000);
        TOFDT0[i] = new TH1D(Form("TOF_%dDT0",i+1),Form("TOF_%dDT0",i+1),1000,-10000,10000);
        TOFUTns[i] = new TH1D(Form("TOFUTns%d",i+1),Form("TOFUTns%d",i+1),162,-3,3);
        TOFDTns[i] = new TH1D(Form("TOFDTns%d",i+1),Form("TOFDTns%d",i+1),162,-3,3);
        TOFMT[i] = new TH1D(Form("TOFMT%d",i+1),Form("TOFMT%d",i+1),162,-3,3);
        TOFUMIP[i] = new TH1D(Form("TOFUMIP%d",i+1),Form("TOFUMIP%d",i+1),1200,0,1200);
        TOFDMIP[i] = new TH1D(Form("TOFDMIP%d",i+1),Form("TOFDMIP%d",i+1),1200,0,1200);
        TOFUBG[i] = new TH1D(Form("TOFUBG%d",i+1),Form("TOFUBG%d",i+1),600,0,600);
...
//        SHist[i] = new TH2D(Form("SHist%d",i+1),Form("SHist%d",i+1),2000,0,2000,100,30,40);
   ...
   double sigma_bh1ubgprm[NumOfSegBH1]; 
   double sigma_bh1dbgprm[NumOfSegBH1]; 
   ...
   double sigma_bh2ubgprm[NumOfSegBH2]; 
   double sigma_bh2dbgprm[NumOfSegBH2]; 
...
/////////////////////////////////////////////////////////////////////////////////////////////
   ...
   for (Long64_t s=0; s<nentries;s++) {
      ...
      for (int i=0; i<NumOfSegTOF;i++) {
        if(tofut[i]>0 && tofdt[i]>0 && bh2nhits == 1){
   ...
   TString pdf = Form("%s/pdf/hdprm/STOF1_%05d.pdf", anadir.Data(),runnum);
...
//////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for(int j =  0; j<2; j++){
    for(int i =0; i<NofProject; i++){
      ...
      TH1D *tmp1 = (TH1D*)SHist[j]->ProjectionY(Form("Projectoin%d",i+1),bin_min, bin_max);
      double center = tmp1->GetBinCenter(tmp1->GetMaximumBin());
      tmp1->Fit("fit","Q","",center - 0.4 ,center + 0.4  );
      tmp1->Draw();
  
      double x_min = SHist[j]->GetXaxis()->GetBinCenter(bin_min);
      double x_max = SHist[j]->GetXaxis()->GetBinCenter(bin_max);
...
//      double ctofmtime = ch2ns*(tofut[10][0] + tofdt[10][0])*0.5 - (a[0][0]/sqrt(a[0][1] + tofua[10]) + a[0][2]) - (a[1][0]/sqrt(a[1][1] + tofda[10]) + a[1][2]);
      ...
      double bh2mtime  = ch2ns*(bh2ut[3][0] + bh2dt[3][0])*0.5;
      double cbh2mtime  = ch2ns*(bh2ut[3][0] + bh2dt[3][0])*0.5 + (0.5*a[2][0]/sqrt(a[2][1] + bh2ua[3]) ) + (0.5*a[3][0]/sqrt(a[3][1] + bh2da[3]) );
