...
// TTreePlayer->SetBranchStatus("branchname",1);  // activate branchname
   ...
   TH1D *T0; 
      for (int i=0; i<NumOfSegBH2;i++) {
        BH2MT[i] = new TH1D(Form("BH2MT%d",i+1),Form("BH2MT%d",i+1),100,-1,1);
      }
        T0 = new TH1D(Form("T0%d",1),Form("T0%d",1),240,-0.6,0.6);
   ...
   TString pdf1 = Form("%s/pdf/trigger_study/mt_t0_%05d.pdf", anadir.Data(),runnum);
   c1->Print(pdf1+"["); 
   ...
   for (int i=0; i<NumOfSegBH2;i++) {
       ...
       BH2MT[i]->GetXaxis()->SetRangeUser(bh2mtm[i]-l_2,bh2mtm[i]+2*l_2);   

   }

   TCanvas *c3 = new TCanvas("c3","c3",800,700); 
       T0->Draw(); 
       T0->GetXaxis()->SetRangeUser(-0.4,0.6);   
  ...
  TString fout1 = (Form( "%s/hp_dat/HodoParam_BH1_corr_MT_%05d.dat", anadir.Data() ,runnum));  
  std::ofstream fout_1(fout1.Data()); 
  for(int i=0; i<NumOfSegBH1; i++){
     fout_1 << bh1mtm[i] << endl;
  }     

  TString fout2 = (Form( "%s/hp_dat/HodoParam_BH2_corr_MT_%05d.dat", anadir.Data() ,runnum));  
  std::ofstream fout_2(fout2.Data()); 
  for(int i=0; i<NumOfSegBH2; i++){
     fout_2 << bh2mtm[i] << endl;
  }     
}
